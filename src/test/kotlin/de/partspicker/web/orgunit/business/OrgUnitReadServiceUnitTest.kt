...
class OrgUnitReadServiceUnitTest : ShouldSpec({
    ...
    context("findAllMembershipsOfCurrentUser") {
        should("return a membership for every org unit the current user holds an entitlement in") {
            // given
            ...
            // when
            val returnedMemberships = cut.findAllMembershipsOfCurrentUser(Pageable.unpaged())

            // then
            returnedMemberships.content shouldContainExactly listOf(OrgUnitMembership.from(entitlementEntity))
        }
        ...
        should("return an empty list when the current user holds no entitlement at all") {
            ...
            // when & then
            cut.findAllMembershipsOfCurrentUser(Pageable.unpaged()).content shouldBe emptyList()
        }
    }
})
