...
class OrgUnitAccessIntTest(
    ...
    context("requests against an unrelated org unit") {
        ...
        should("return status 403 for the workflow state of one of its projects") {
            mockMvc.get("/projects/${UnrelatedOrgUnit.PROJECT_ID}/instance/node")
                .andExpect { status { isForbidden() } }
        }

        should("return status 403 when advancing the workflow state of one of its projects") {
            mockMvc.post("/projects/${UnrelatedOrgUnit.PROJECT_ID}/instance/edges/1")
                .andExpect { status { isForbidden() } }
        }
