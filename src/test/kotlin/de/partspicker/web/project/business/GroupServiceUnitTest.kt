...
class GroupServiceUnitTest : ShouldSpec({
    ...
    context("getById") {
        should("return the group with the given id") {
            ...
            every { orgUnitAccessServiceMock.requireAtLeast(groupEntity.orgUnit.id, AccessLevel.READ) } just runs
    ...
    context("findAllForOrgUnit") {
        should("return every group of the given org unit") {
            ...
            val orgUnitId = 1L
            val groupEntity = GroupEntityGenerators.generator.next()
            every { orgUnitAccessServiceMock.requireAtLeast(orgUnitId, AccessLevel.READ) } just runs
            every {
    ...
    context("create") {
        should("store the group within the given org unit & return it") {
            ...
            val orgUnitId = 1L
            val groupEntity = GroupEntityGenerators.generator.next()
            every { orgUnitAccessServiceMock.requireAtLeast(orgUnitId, AccessLevel.CONFIGURE) } just runs
            every { creationInfoFactoryMock.forCurrentUser() } returns groupEntity.creation
        ...
        should("name the current user as the creator of the stored group") {
            // given
            val orgUnitId = 1L
            val creation = CreationInfoGenerators.generator.next()
            val groupEntity = GroupEntityGenerators.generator.next()
            ...
            savedGroup.captured.creation shouldBe creation
        }
        ...
        should("refuse & not store when the caller may not edit the given org unit") {
            ...
            val orgUnitId = 1L
            every {
                orgUnitAccessServiceMock.requireAtLeast(orgUnitId, AccessLevel.CONFIGURE)
            } throws OrgUnitAccessDeniedException(orgUnitId, AccessLevel.CONFIGURE)
    ...
    context("update") {
        should("update name & description of the group with the given id") {
            ...
            val groupEntity = GroupEntityGenerators.generator.next()
            every { groupRepositoryMock.findByIdOrNull(groupEntity.id) } returns groupEntity
            every { orgUnitAccessServiceMock.requireAtLeast(groupEntity.orgUnit.id, AccessLevel.CONFIGURE) } just runs
        ...
        should("refuse & not store when the caller may not edit the org unit of the group") {
            ...
            every { groupRepositoryMock.findByIdOrNull(groupEntity.id) } returns groupEntity
            every {
                orgUnitAccessServiceMock.requireAtLeast(groupEntity.orgUnit.id, AccessLevel.CONFIGURE)
            } throws OrgUnitAccessDeniedException(groupEntity.orgUnit.id, AccessLevel.CONFIGURE)
        ...
        should("detach every project of the group & then delete it") {
            ...
            every {
                orgUnitAccessServiceMock.requireMemberCreatorOrAtLeast(
                    groupEntity.orgUnit.id,
                    groupEntity.creation.createdBy.id,
                    AccessLevel.MAINTAIN
                )
            } just runs
            every { projectServiceMock.removeGroupForAllById(groupEntity.id) } just runs
            every { groupRepositoryMock.delete(groupEntity) } just runs
