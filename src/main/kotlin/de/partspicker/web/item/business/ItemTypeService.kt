...
class ItemTypeService(
    ...
    fun create(itemTypeToCreate: CreateItemType): ItemType {
        this.orgUnitAccessService.requireAtLeast(itemTypeToCreate.orgUnitId, AccessLevel.CONFIGURE)

        val createdItemTypeEntity = this.itemTypeRepository.save(
            ItemTypeEntity(
                name = itemTypeToCreate.name,
                description = itemTypeToCreate.description,
                orgUnit = this.orgUnitRepository.getReferenceById(itemTypeToCreate.orgUnitId),
    ...
    fun update(id: Long, name: String, description: String?): ItemType {
        val itemTypeEntity = this.getItemTypeOrThrow(id)
        this.orgUnitAccessService.requireAtLeast(itemTypeEntity.orgUnit.id, AccessLevel.CONFIGURE)
