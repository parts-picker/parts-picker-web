...
fun generateGetAllMembershipsOfCurrentUserLink(relation: LinkRelation) =
    generateGetAllMembershipsOfCurrentUserLink(relation.toString())

fun generateGetAllMembershipsOfCurrentUserLink(relation: String): Link {
    val uriTemplate = UriTemplate.of(
        linkTo<OrgUnitController> { handleGetAllMembershipsOfCurrentUser(Pageable.unpaged()) }.toUri().toString()
