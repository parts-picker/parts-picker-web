...
@Component
class AssignedItemResourceAssembler(
    private val orgUnitAccessService: OrgUnitAccessService
) : RepresentationModelAssembler<AssignedItem, AssignedItemResource> {
    ...
    private fun generateDefaultLinks(assignedItem: AssignedItem): List<Link> {
        return LinkListBuilder()
            .with(
                ...
                this.orgUnitAccessService.atLeast(assignedItem.orgUnitId, USE)
            ).build()
    }
}
