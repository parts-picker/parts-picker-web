...
class ProjectService(
    ...
    fun create(project: CreateProject): Project {
        this.orgUnitAccessService.requireAtLeast(project.orgUnitId, AccessLevel.USE)

        val groupEntity = project.groupId?.let { groupId -> this.getGroupOfOrgUnitOrThrow(groupId, project.orgUnitId) }
        ...
        val savedProjectEntity = this.projectRepository.save(
            ProjectEntity(
                ...
                orgUnit = this.orgUnitRepository.getReferenceById(project.orgUnitId),
    ...
    fun copy(sourceProjectId: Long, name: String): Project {
        ...
        this.orgUnitAccessService.requireAtLeast(sourceProjectEntity.orgUnit.id, AccessLevel.USE)

        val sourceProject = Project.from(sourceProjectEntity)
        val createProject = CreateProject(
            orgUnitId = sourceProjectEntity.orgUnit.id,
            name = name,
    ...
    private fun getGroupOfOrgUnitOrThrow(groupId: Long, orgUnitId: Long): GroupEntity {
        ...
        SameOrgUnitRule(orgUnitId, groupEntity.orgUnit.id).valid()
